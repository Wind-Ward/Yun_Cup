...
if __name__ == '__main__':
    ...
    for i, clf in enumerate(base_models):
        ...
        def write():
            ...
            res['Id'] = list(test_id)
            res['pre'] = list(s)
            res.to_csv('./output/lgb_custom_w2v_v5_stacking_200_ndim.csv', index=False, header=False)
            res["pre"] = round_score(res["pre"])

            res.to_csv('./output/lgb_custom_w2v_v5_stacking_200_ndim_round_score.csv', index=False, header=False)


        write()
    ## 保存元特征
    ## S_train_v1的维度要和S_test_v1的一致
    pickle.dump(S_train, open('./output/S_train_custom_w2v_220000_lgb_v5_200_ndim_stacking.pkl', 'wb'))
    pickle.dump(S_test, open('./output/S_test_custom_w2v_50000_lgb_v5_200_ndim_stacking.pkl', 'wb'))
    print("X shape:", X.shape)
